...
struct wasm_func_t : public wasm_externtype_base_t<wasm::Func>,
  ...
  wasm_func_t(wasm::Store *s) noexcept
...
struct wasm_global_t : public wasm_externtype_base_t<wasm::Global>,
  ...
  wasm_global_t(wasm::Store *s, const wasm::GlobalType &gt,
                const wasm::Val &) noexcept
      : wasm_ref_base_t<wasm_global_t>(s),
...
inline wasm::Val conv_val_to_cpp(wasm_val_t v) {
  switch (v.kind) {
...
wasm_func_new_with_env(wasm_store_t *store, const wasm_functype_t *type,
                       wasm_func_callback_with_env_t callback, void *,
                       void (*)(void *)) {
  if (store && type && callback) {
    // TODO: implement this
  }
...
wasm_global_new(wasm_store_t *store, const wasm_globaltype_t *type,
  ...
  if (store && type && val) {
    auto cppval = conv_val_to_cpp(*val);
    return static_cast<wasm_global_t *>(
        wasm::Global::make(store, type, cppval).release());
  }
  return nullptr;
}
...
wasm::own<wasm::Ref> wasm::Table::get(wasm::Table::size_t) const {
  // TODO: implement this
  return wasm::own<wasm::Ref>();
}

bool wasm::Table::set(wasm::Table::size_t, const wasm::Ref *) {
  // TODO: implement this
  return false;
}
